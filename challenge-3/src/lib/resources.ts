 ...
 * @returns `true` if the domain is "available" for purchase. Note that this
...
export async function isDomainAvailable(domain: string): Promise<boolean> {
  ...
  // NOTE: Same functionality as above, but without the sleep.
  // Sleep wasn't correctly importing and this function couldn't be called.
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve(domain.toLowerCase().charCodeAt(0) % 2 === 0);
    }, 1000);
  });
}
